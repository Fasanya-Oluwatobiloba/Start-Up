const LevelFour = () => {
	return (
	  <div className="flex justify-center py-10 px-4">
		<div className="bg-purple-200 rounded-xl lg:max-w-lg md:max-w-md max-w-sm w-full">
		  <h1 className="text-3xl rounded-t-xl pl-4 py-2 bg-purple-600 text-white font-bold">
			400 LEVEL
		  </h1>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS4012</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 mt-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
  
		  <div className="flex items-center justify-between text-black rounded-3xl py-2 px-3 bg-white mx-2 my-3">
			<div className="">
			  <h2 className="text-base font-bold">MTS 401</h2>
			  <p className="text-sm">1st Semester</p>
			</div>
			<p className="rotate-90">▼</p>
		  </div>
		</div>
	  </div>
	);
  };
  
  export default LevelFour;
  